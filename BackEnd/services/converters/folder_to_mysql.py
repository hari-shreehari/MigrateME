...
def convert(spark, request):
    ...
    if not (request.output.url and request.output.properties):
        raise HTTPException(400, "output url and properties are required for mysql output")

    input_folder = request.input.path
    
    try:
        build_module = importlib.import_module(f".services.build.mysql", package="BackEnd")
        build_function = getattr(build_module, "write_mysql")
    except (ImportError, AttributeError):
        raise HTTPException(400, f"Unsupported output type: mysql")

    output_url = request.output.url
    output_properties = request.output.properties
